...
const structuredData = {
  ...
  "@type": "WebPage",
  name: "LeetCode 75 Study Plan - DevExCode",
  url: "https://devexcode.com/interview/leetcode-75",
  ...
  keywords: `LeetCode 75, coding interview prep, technical interviews, algorithms, data structures, DevExCode, LeetCode, Array, Two Pointers, Dynamic Programming, ${leetcode75.sections.map((s) => s.name).join(", ")}`,
  author: {
    "@type": "Organization",
    name: "DevExCode",
  },
  ...
  image: "https://devexcode.com/og-image-leetcode75.jpg",
  breadcrumb: {
    "@type": "BreadcrumbList",
    itemListElement: [
      {
      ...
      },
      {
        ...
        item: "https://devexcode.com/interview/leetcode-75",
      },
    ],
  },
  mainEntity: [
    {
      "@type": "ItemList",
      name: "LeetCode 75 Problems",
      description: "A curated list of 75 coding problems to prepare for technical interviews, covering key algorithms and data structures.",
      numberOfItems: leetcode75.sections.reduce((sum, section) => sum + section.questions.length, 0),
      itemListElement: leetcode75.sections.flatMap((section, sectionIndex) =>
        section.questions.map((question, qIndex) => ({
          ...
          item: {
            ...
            description: `A ${question.difficulty.toLowerCase()} coding problem in the ${section.name} category of the LeetCode 75 Study Plan.`,
    ...
    },
    {
      ...
      mainEntity: [
        {
          "@type": "Question",
          name: "What is the LeetCode 75 Study Plan?",
          acceptedAnswer: {
            "@type": "Answer",
            text: "The LeetCode 75 Study Plan is a curated list of 75 coding problems designed to cover essential algorithms and data structures for technical interview preparation.",
          },
        },
        {
          "@type": "Question",
          name: "How to track progress in LeetCode 75?",
          acceptedAnswer: {
            "@type": "Answer",
            text: "Log in to DevExCode to track your progress across the LeetCode 75 problems. Mark problems as solved to see your completion percentage in real-time.",
          },
        },
        {
          "@type": "Question",
          name: "Are solutions available for LeetCode 75 problems?",
          acceptedAnswer: {
            "@type": "Answer",
...
export default function LeetCode75Page({ totalQuestions }) {
  const { data: session, status } = useSession();
  const [isLoggedIn, setIsLoggedIn] = useState(!!session);
  const [userName, setUserName] = useState(session?.user?.name || "");
  const [filter, setFilter] = useState("all");
  const [filteredSections, setFilteredSections] = useState(leetcode75.sections);
  const [progress, setProgress] = useState(0);
  const [solvedProblems, setSolvedProblems] = useState([]);
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(false);

  // Extract LeetCode 75 question IDs
  const leetCode75Ids = new Set(
    leetcode75.sections.flatMap((section) =>
  ...
  // Fetch progress for logged-in users and filter for LeetCode 75 questions
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        ...
        const solved = (data.solved || [])
          .map(Number)
          .filter((id) => leetCode75Ids.has(id));
  ...
  useEffect(() => {
    if (filter === "all") {
      setFilteredSections(leetcode75.sections);
    } else {
      setFilteredSections(
        leetcode75.sections.filter((section) =>
  ...
  // Lazy load sections
  const sectionRefs = leetcode75.sections.map(() =>
      ...
      <Layout isLoggedIn={false} userName="" isLoginModalOpen={isLoginModalOpen} setIsLoginModalOpen={setIsLoginModalOpen}>
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-4">Loading LeetCode 75...</p>
          </div>
        </div>
      </Layout>
    );
  ...
  const stats = [
    ...
    { label: "Topics Covered", value: leetcode75.sections.length },
    {
      label: "Difficulty Levels",
      value: ["Easy", "Medium", "Hard"].filter((level) =>
        leetcode75.sections.some((section) =>
  ...
  return (
    ...
    >
      <Head>
        <title>LeetCode 75 Study Plan - DevExCode ({totalQuestions} Problems)</title>
        <meta
          name="description"
          content={`Master the LeetCode 75 Study Plan with ${totalQuestions} curated coding problems to ace technical interviews. Organized by Array, Two Pointers, Dynamic Programming, and more.`}
        />
        <meta
          name="keywords"
          content={`LeetCode 75, coding interview prep, technical interviews, algorithms, data structures, DevExCode, LeetCode, Array, Two Pointers, Dynamic Programming, ${leetcode75.sections.map((s) => s.name).join(", ")}`}
        />
        <meta name="author" content="DevExCode Team" />
        <meta name="robots" content="index, follow" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta property="og:title" content={`LeetCode 75 Study Plan - DevExCode (${totalQuestions} Problems)`} />
        <meta
          property="og:description"
          content={`Prepare for coding interviews with ${totalQuestions} essential problems in the LeetCode 75 Study Plan, organized by topics like Array, Two Pointers, and Dynamic Programming.`}
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://devexcode.com/interview/leetcode-75" />
        <meta property="og:image" content="https://devexcode.com/og-image-leetcode75.jpg" />
        <meta property="og:image:alt" content="LeetCode 75 Study Plan on DevExCode" />
        <meta property="og:site_name" content="DevExCode" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`LeetCode 75 Study Plan - DevExCode (${totalQuestions} Problems)`} />
        <meta
          name="twitter:description"
          content={`Master technical interviews with ${totalQuestions} curated coding problems in the LeetCode 75 Study Plan.`}
        />
        <meta name="twitter:image" content="https://devexcode.com/twitter-image-leetcode75.jpg" />
        <meta name="twitter:image:alt" content="LeetCode 75 Study Plan on DevExCode" />
        <meta name="twitter:creator" content="@DevCodeEx" />
        <link rel="canonical" href="https://devexcode.com/interview/leetcode-75" />
          ...
          >
            <Link href="#problems" prefetch={true}>
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                aria-label="Explore LeetCode 75 problems"
              >
                Start Solving
              </motion.button>
            </Link>
            {isLoggedIn && (
              <Link href="/profile" prefetch={true}>
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  aria-label="View your LeetCode 75 progress"
                >
                  View Progress
                </motion.button>
              </Link>
          ...
          <div className="flex justify-center space-x-4">
            {["all", "easy", "medium", "hard"].map((level) => (
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                aria-label={`Filter LeetCode 75 by ${level} difficulty`}
              >
                {level}
              </motion.button>
            ))}
      ...
      <section className="py-16 bg-indigo-50 dark:bg-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            Start the LeetCode 75 Study Plan today and build the skills needed to excel in coding interviews.
          </motion.p>
          <motion.div
            variants={sectionVariants}
            initial="hidden"
          ...
          >
            <Link href="#problems" prefetch={true}>
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                aria-label="Start solving LeetCode 75 problems"
              >
                Start Solving
              </motion.button>
            </Link>
...
export async function getServerSideProps() {
  return {
    props: {
      totalQuestions: leetcode75.sections.reduce(
