...
export default function PotdIndex({ initialGfgPotd, initialLeetcodePotd, error }) {
    ...
    // Find POTD for today from local data if not provided
    const defaultGfgPotd = gfgPotdData.find((potd) => potd.date === today) || null;
    const defaultLeetcodePotd = leetcodePotdData.find((potd) => potd.date === today) || null;

    const [gfgPotd, setGfgPotd] = useState(initialGfgPotd || defaultGfgPotd);
    const [leetcodePotd, setLeetcodePotd] = useState(initialLeetcodePotd || defaultLeetcodePotd);
