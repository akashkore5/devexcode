...
export default function MicroDevTipsDetailClient({ frontmatter, content, relatedTips }) {
  ...
  const fetchCsrfToken = useCallback(async (retries = 3, delay = 1000) => {
    try {
      ...
      console.error("[MicroDevTipsDetailClient] Error fetching CSRF token:", error.message, error.stack);
      if (retries > 0) {
  ...
  const handleSubmitContribution = async () => {
    ...
    try {
      ...
      console.error("[MicroDevTipsDetailClient] Error submitting contribution:", error);
