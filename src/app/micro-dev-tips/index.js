...
export default function DevTips({ initialViewMode = "table" }) {
  ...
  const [solvedTips, setSolvedTips] = useState([]);
  const [taggedTips, setTaggedTips] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        ...
        setSolvedTips(solved);
        setTaggedTips(tagged);
      } catch (error) {
  ...
  const allTags = useMemo(() => {
    const tags = new Set();
    devTips.forEach((tip) => tip.tags?.forEach((t) => tags.add(t)));
  ...
  const tagCounts = useMemo(() => {
    const counts = {};
    devTips.forEach((tip) =>
      tip.tags?.forEach((t) => (counts[t] = (counts[t] || 0) + 1))
    );
    return counts;
  }, []);
    ...
    if (search) {
      result = result.filter(
        (tip) =>
          ...
          tip.id.toString() === search
      );
    }
    if (difficulty && difficulty !== "All") {
      result = result.filter((tip) => tip.difficulty === difficulty);
    }
    if (tag && tag !== "All") {
      result = result.filter((tip) =>
        tip.tags?.some((t) => t.toLowerCase() === tag.toLowerCase())
      );
    }
    return result;
  ...
  const totalPages = Math.ceil(sortedTips.length / perPage);
  const paginated = sortedTips.slice((page - 1) * perPage, page * perPage);
    ...
    async (id, isSolved) => {
      ...
      try {
        ...
        if (response.ok) {
          setSolvedTips((prev) =>
            isSolved
              ? prev.filter((tipId) => tipId !== numericId)
              : [...prev, numericId]
          );
          toast.success(isSolved ? "Tip unmarked as solved" : "Tip marked as solved");
        } else {
  ...
  const handleTagTip = useCallback(
    async (id, isTagged) => {
      if (status !== "authenticated") {
        setIsLoginModalOpen(true);
        return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast.success(isTagged ? "Tip untagged" : "Tip tagged");
        } else {
  ...
  const canonicalUrl = `https://devexcode.com/micro-dev-tips${tag ? `?tag=${encodeURIComponent(tag)}` : ""}`;
  ...
  return (
    ...
    >
      <Head>
        <title>
          {`Micro Dev Tips - ${search || "All"} Topics | DevCodeEx`}
        </title>
        <meta
          name="description"
          content={`Explore ${search || "all"} development tips with in-depth technical insights. Filter by ${difficulty || "all difficulties"} and ${tag || "all topics"}.`}
        />
        <meta
          name="keywords"
          content={`development tips, technical insights, ${difficulty || "easy medium hard"}, ${tag || "all topics"}, software engineering`}
        />
        ...
        <meta
          property="og:title"
          content={`Micro Dev Tips - ${search || "All"} Topics | DevCodeEx`}
        />
        <meta
          property="og:description"
          content={`Master ${search || "all"} development tips with detailed technical analysis and examples.`}
        />
        ...
        <meta
          ...
          content={`Learn ${search || "all"} development tips with filters for ${difficulty || "all difficulties"} and ${tag || "all topics"}.`}
        />
        <meta
          name="twitter:image"
          ...
          dangerouslySetInnerHTML={{
            __html: DOMPurify.sanitize(
              JSON.stringify({
                ...
                "@type": "ItemList",
                "url": canonicalUrl,
                "name": `Micro Dev Tips ${search || "all"} topics`,
                "description": `Explore ${search || "all"} development tips with in-depth technical insights.`,
                "itemListElement": filtered.slice(0, 10).map((tip, index) => ({
                  "@type": "ListItem",
                  "position": index + 1,
                  "url": `https://devexcode.com/micro-dev-tips/${tip.id}`,
                  "name": `${tip.id}. ${tip.title}`,
                  "description": `Difficulty: ${tip.difficulty}, Tags: ${tip.tags?.join(", ") || ""}`,
                })),
              })
            ),
          }}
            ...
            <div className="flex flex-row items-center gap-2 w-full sm:w-auto">
              <div className="relative flex-grow sm:w-36" ref={perPageRef}>
                <button
                  ...
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-gray-100 text-left flex justify-between items-center transition-all duration-300 shadow-sm hover:shadow-md text-sm truncate"
                  aria-label="Select tips per page"
                >
                  ...
                  <span className="flex-shrink-0 w-4 h-4">
                    {perPage !== 15 ? (
                      <XMarkIcon
                        ...
                        aria-label="Reset tips per page"
                      />
                    ) : (
                      <svg
              ...
              <ul className="space-y-6">
                {paginated.length === 0 ? (
                  <li className="text-center text-gray-500 dark:text-gray-400 py-10 text-lg">
                    No dev tips found. Try adjusting your filters.
                  </li>
                ) : (
                  paginated.map(({ id, title, difficulty: diff, tags }) => (
                    <motion.li
                      ...
                      onClick={() => router.push(`/micro-dev-tips/${id}`)}
                    >
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between">
                          <Link
                            href={`/micro-dev-tips/${id}`}
                          ...
                          <motion.button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleMarkSolved(id, solvedTips.includes(Number(id)));
                            }}
                            className={`rounded-full p-1 ${
                              solvedTips.includes(Number(id))
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          {tags?.map((t, i) => (
                            <Link
                              key={i}
                              href={`/micro-dev-tips?tag=${encodeURIComponent(t)}`}
                        ...
                        <div className="flex gap-3 mt-4">
                          ...
                          <motion.button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleTagTip(id, taggedTips.includes(Number(id)));
                            }}
                            className={`flex items-center px-4 py-2 rounded-lg shadow-md text-white transition-transform duration-200 ${
                              taggedTips.includes(Number(id))
                            ...
                            whileTap={{ scale: 0.95 }}
                            aria-label={
                              taggedTips.includes(Number(id)) ? "Untag tip" : "Tag tip"
                            }
                          >
                          ...
                          >
                            <td className="px-4 py-4 w-12">
                              <motion.button
                                ...
                                onClick={() =>
                                  handleMarkSolved(
                                    tip.id,
                                    solvedTips.includes(Number(tip.id))
                                  )
                                }
                                className={`rounded-full p-1 ${
                                  solvedTips.includes(Number(tip.id))
                              ...
                              <div className="flex flex-wrap gap-2 sm:gap-1">
                                <button
                                  onClick={() => {
                                    ...
                                    setPage(1);
                                    updateQuery({ ...router.query, difficulty: tip.difficulty });
                                  }}
                                  ...
                                  style={{
                                    backgroundColor: getDifficultyColor(tip.difficulty).bg,
                                    color: getDifficultyColor(tip.difficulty).text,
                                  }}
                                  title={tip.difficulty}
                                  aria-label={`Filter by ${tip.difficulty} difficulty`}
                                >
                                  {tip.difficulty}
                                </button>
                              </div>
                            ...
                            <td className="px-4 py-4 w-12">
                              <motion.button
                                ...
                                onClick={() =>
                                  handleTagTip(
                                    tip.id,
                                    taggedTips.includes(Number(tip.id))
                                  )
                                }
                                className={`p-1 ${
                                  taggedTips.includes(Number(tip.id))
                            ...
                            <td className="px-4 py-4 w-12">
                              <Link
                                ...
                                aria-label={`View article for ${tip.title}`}
                              >
          ...
          >
            <div className="text-sm text-gray-600 dark:text-gray-300">
              <span>
                ...
                {Math.min(page * perPage, sortedTips.length)} of{" "}
