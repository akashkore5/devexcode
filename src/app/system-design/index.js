...
export default function SystemDesign({ initialViewMode = "table" }) {
  ...
  const canonicalUrl = `https://devexcode.com/system-design${tag ? `?tag=${encodeURIComponent(tag)}` : ""}`;
            ...
            __html: DOMPurify.sanitize(
              JSON.stringify({
                ...
                "itemListElement": filtered.slice(0, 10).map((q, index) => ({
                  "@type": "ListItem",
                  "position": index + 1,
                  "url": `https://devexcode.com/system-design/${q.id}-${generateSlug(q.title)}`,
                  "name": `${q.id}. ${q.title}`,
                ...
                {paginated.length === 0 ? (
                  ...
                  paginated.map(({ id, title, difficulty: diff, tags }) => (
                    <motion.li
                      ...
                      onClick={() => router.push(`/system-design/${id}-${generateSlug(title)}`)}
                    >
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between">
                          <Link
                            href={`/system-design/${id}-${generateSlug(title)}`}
                        ...
                        paginated.map((question) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-4 text-sm">
                              <Link
                                href={`/system-design/${question.id}-${generateSlug(question.title)}`}
