...
export default function DatabaseArticle({ frontmatter, content, relatedTopics }) {
  ...
  const fetchCsrfToken = useCallback(async (retries = 3, delay = 1000) => {
    try {
      ...
      console.error("[DatabaseArticle] Error fetching CSRF token:", error.message, error.stack);
      if (retries > 0) {
        console.log(`[DatabaseArticle] Retrying CSRF fetch (${retries} attempts left)...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
        return fetchCsrfToken(retries - 1, delay * 2);
      }
      console.error("[DatabaseArticle] CSRF token fetch failed after retries");
      setCsrfError(true);
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        ...
        console.error("[DatabaseArticle] Error fetching progress:", error);
    ...
    const checkAndMarkViewed = async () => {
      try {
        const response = await fetch("/api/user/progress/check", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ type: "databases", action: "viewed", id }),
          credentials: "include",
        });
        const data = await response.json();
        ...
        if (!markResponse.ok) console.error("[DatabaseArticle] Failed to mark viewed:", markResponse.statusText);
      } catch (error) {
        console.error("[DatabaseArticle] Error checking/marking viewed:", error);
      }
    };
  ...
  const handleMarkCompleted = useCallback(async () => {
    ...
    try {
      ...
      console.error("[DatabaseArticle] Error updating completion status:", error);
  ...
  const handleSubmitContribution = async () => {
    ...
    try {
      console.log("[DatabaseArticle] Submitting contribution with payload:", {
        articleId: id,
        url: `https://devexcode.com/databases/${id}`,
      ...
      if (response.ok) {
        ...
        setContributionContent(content); // Reset to original content
      } else if (response.status === 403 && data.message === "Invalid CSRF token") {
        
        const newCsrfToken = await fetchCsrfToken();
        if (newCsrfToken) {
          // Retry submission with new CSRF token
          ...
          if (retryResponse.ok) {
            ...
            console.error("[DatabaseArticle] Retry failed:", retryData.message);
        ...
        console.error("[DatabaseArticle] Contribution submission failed:", data.message);
        toast.error(data.message || "Failed to submit contribution");
      }
    } catch (error) {
      console.error("[DatabaseArticle] Error submitting contribution:", error.message, error.stack);
  ...
  const structuredData = {
    ...
    "@type": "Article",
    headline: frontmatter.title || "Database Developer Guide",
    description: frontmatter.description || "Comprehensive guide for database developers covering essential topics.",
    keywords: `Database, ${frontmatter.title}, ${frontmatter.tags.join(", ")}, programming, data management, ${frontmatter.difficulty.toLowerCase()}`,
    author: {
      "@type": "Organization",
      name: "DevCodeEx",
    },
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://devexcode.com/databases/${id}`,
    },
    image: [
      `https://devexcode.com/database-${id}-guide.png`,
      "https://devexcode.com/database-guide.png",
    ],
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
        ...
        },
        {
          ...
          item: "https://devexcode.com/databases",
        },
        {
          "@type": "ListItem",
          position: 3,
          name: frontmatter.title,
          item: `https://devexcode.com/databases/${id}`,
        },
      ],
    },
  };
  ...
  return (
    <Layout
      ...
      description={frontmatter.description || "Comprehensive guide for database developers."}
    ...
    >
      <Head>
        <meta
          name="keywords"
          content={`Database, ${frontmatter.title}, ${frontmatter.tags.join(", ")}, programming, data management, ${frontmatter.difficulty.toLowerCase()}`}
        />
        <meta
          name="description"
          content={frontmatter.description || "Comprehensive guide for database developers."}
        />
        ...
        <meta
          property="og:url"
          content={`https://devexcode.com/databases/${id}`}
        />
        <meta
          property="og:image"
          content={`https://devexcode.com/database-${id}-guide.png`}
        />
        <meta
          property="og:image:alt"
          content={`Database guide for ${frontmatter.title}`}
        />
        ...
        <link
          rel="canonical"
          href={`https://devexcode.com/databases/${id}`}
        />
        ...
        >
          ...
          >
            <Link
              ...
              aria-label="Back to Database Learning Path"
            >
              <ChevronLeftIcon className="w-4 h-4 mr-1" />
              Back to Database Learning Path
            </Link>
          </motion.nav>
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              {frontmatter.tags.map((tag) => (
                <Link
                  key={tag}
                  href={`/databases?tag=${encodeURIComponent(tag)}`}
...
export async function getStaticProps({ params }) {
  try {
    const filePath = path.join(process.cwd(), "dailyblogsdb", `${params?.id}.md`);
    ...
    try {
      ...
      frontmatter = {
        ...
        description: data.description || "Comprehensive guide for database developers.",
      ...
      const allTopics = databaseTopics.topics.flatMap((t) => [t, ...t.subtopics]);
      ...
      frontmatter = {
        ...
        description: currentTopic.description || "Comprehensive guide for database developers.",
      ...
      content = `## Placeholder Content\n\nThis database topic blog is under construction. Please check back later.`;
    }
